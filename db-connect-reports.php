...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['files'])) {
    // Ensure $_POST['files'] is treated as a string
    $fileIds = is_array($_POST['files']) ? implode(',', $_POST['files']) : $_POST['files'];
    $fileIdsArray = explode(',', $fileIds);
    $deletedFiles = 0;

    foreach ($fileIdsArray as $fileId) {
        // Ensure $fileId is sanitized and is a number
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            if (unlink($fullPath)) {
                ...
                $stmtDelete = $conn->prepare("DELETE FROM files WHERE id = ?");
                ...
                $stmtDelete->close(); // Close the delete statement
            } else {
